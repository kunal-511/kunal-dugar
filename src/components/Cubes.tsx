  ...
  return (
    <div className="relative w-1/2 max-md:w-11/12 aspect-square" style={wrapperStyle}>
      <div ref={sceneRef} className="grid w-full h-full" style={sceneStyle}>
        {cells.map((_, r) =>
          cells.map((__, c) => {
            const cubeIndex = r * gridSize + c;
            const skill = skills[cubeIndex % skills.length];
            const SkillIcon = skill?.icon;

            return (
              <div
                key={`${r}-${c}`}
                className="cube relative w-full h-full aspect-square [transform-style:preserve-3d] group/cube"
                data-row={r}
                data-col={c}
              >
                ...
                {/* Tooltip */}
                {skill && (
                  <div className="absolute -top-12 left-1/2 -translate-x-1/2 opacity-0 group-hover/cube:opacity-100 transition-opacity duration-300 pointer-events-none z-50 whitespace-nowrap">
                    <div className="px-3 py-2 rounded-lg bg-black/90 backdrop-blur-sm border border-primary/30 shadow-xl">
